...
describe("GIVEN a DendronEngineClient running on client-side", () => {
  describe("WHEN writing a new note", () => {
    test("THEN create event fired with the correct props AND update note event fired for its parent", () =>
      new Promise((done) => {
        runEngineTestV5(
  ...
  describe("WHEN writing a new note as a grandchild, whose parent doesn't exist yet", () => {
    test("THEN create event fired for both child and parent (with stub), and update note fired for grandparent", () =>
      new Promise((done) => {
        runEngineTestV5(
          async ({ engine, vaults, wsRoot }) => {
            const engineClient = engine as DendronEngineClient;
  ...
  describe("WHEN writing over an existing note by changing its ID", () => {
    test("THEN expect all dependent notes to be updated", () =>
      new Promise((done) => {
        runEngineTestV5(
          async ({ engine }) => {
            const engineClient = engine as DendronEngineClient;
  ...
  describe("WHEN calling bulkWriteNotes", () => {
    test("THEN onNoteCreate event fires for each added note", () =>
      new Promise((done) => {
        runEngineTestV5(
          async ({ engine, vaults, wsRoot }) => {
            const engineClient = engine as DendronEngineClient;
  ...
  describe("WHEN updating a note (no links)", () => {
    test("THEN update event fired with the updated title", () =>
      new Promise((done) => {
        runEngineTestV5(
          async ({ engine, vaults }) => {
            const engineClient = engine as DendronEngineClient;
  ...
  describe("WHEN deleting a note that has children", () => {
    test("THEN *update* (not delete) event fired with stub === true", () =>
      new Promise((done) => {
        runEngineTestV5(
          async ({ engine }) => {
            const engineClient = engine as DendronEngineClient;
  ...
  describe("WHEN deleting a note that has no children", () => {
    test("THEN delete event fired with the correct note props AND update event fired for its parent", () =>
      new Promise((done) => {
        runEngineTestV5(
          async ({ engine }) => {
            const engineClient = engine as DendronEngineClient;
  ...
  describe("WHEN renaming an existing note", () => {
    test("THEN create AND delete events both get fired with the correct props", () =>
      new Promise((done) => {
        runEngineTestV5(
          async ({ engine, vaults }) => {
            const engineClient = engine as DendronEngineClient;
